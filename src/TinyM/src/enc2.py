...
class EncoderToOdometry:
    ...
    def __init__(self):
        ...
        self.enc_sub_tm1 = rospy.Subscriber('encoder_pos_tm1', Float64, self.enc_tm1_callback)
        self.enc_sub_tm2 = rospy.Subscriber('encoder_pos_tm2', Float64, self.enc_tm2_callback)
    ...
    def calculate_odometry(self):
        ...
        # Calculate linear and angular displacement from encoder data
        linear_disp = (self.enc_pos_tm1 + self.enc_pos_tm2) / 2.0
        angular_disp = (self.enc_pos_tm2 - self.enc_pos_tm1) / 0.67  # Assuming axle length is 0.67
        ...
        # Update pose
        self.pose.position.x += linear_disp * math.cos(self.pose.orientation.z)
        self.pose.position.y += linear_disp * math.sin(self.pose.orientation.z)

        # Update quaternion (orientation)
        quaternion = tf.quaternion_from_euler(0, 0, self.pose.orientation.z + angular_disp)
